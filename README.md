...
0.x, a spike. `read_bytes(&[u8])` (and `read_str(&str)` for text that is already Unicode) reads LINE, CIRCLE, ARC, ELLIPSE, LWPOLYLINE, POLYLINE (2D, 3D, polyface and polygon meshes, with their vertices), SPLINE (its degree, knots, weights and control or fit points), TEXT, MTEXT (with its attachment point), ATTRIB, ATTDEF, INSERT (with its attributes and extrusion), DIMENSION (as a reference to its anonymous block), LEADER, MULTILEADER (its leader lines), POINT, SOLID, TRACE, 3DFACE, HATCH (its boundary paths, fill, pattern lines, gradient, elevation and extrusion), VIEWPORT (its frame on the sheet), RAY, XLINE, TOLERANCE (its frame's text as written), WIPEOUT (its clip polygon, taken from the image's pixel space), IMAGE (its frame, display settings, clip boundary and the image definition it names), ACAD_TABLE (as the block reference it is, its rotation taken from the table's horizontal direction), MLINE (its points, miters and style) and LIGHT, the LAYER and DIMSTYLE tables, the multiline styles, layouts and image definitions of the OBJECTS section and the BLOCKS section (what the model and paper space blocks own is the drawing's own entity list); every other entity type is kept as `UNKNOWN`. What the LibreDWG test corpus says about it is in [CAVEATS.md](CAVEATS.md): 66 of its 67 DXF files read, and the same drawing saved as 2000 through 2018 reads to the same 72 entities from every version. The model's golden cases that have a copy here ([tests/golden/README](tests/golden/README.md) lists them) are read and compared with their expected models: exactly for most, through the properties their own tests name for the rest. A reference to a block the file does not define is kept as an unresolved reference carrying the name. Binary DXF is not read.
