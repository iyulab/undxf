...
- **Bulges** (DXF 42) are carried on each LWPOLYLINE and 2D POLYLINE vertex; an absent 42 is a straight segment. A 42 written before an LWPOLYLINE's first vertex belongs to no vertex and is ignored.
- **VIEWPORT** is read as its frame on the sheet -- its center (10), width (40) and height (41); what it looks at in model space is not carried. **RAY** and **XLINE** are read as their base point (10) and direction (11).
- **HATCH** is read in order, since its record repeats group codes: its boundary paths (polyline paths with their bulges, or edges -- lines, arcs, ellipses and splines by their control points), its fill flag, its pattern lines and its gradient. Edge angles and pattern angles, written in degrees, are carried in radians. A record that runs out before a count it states is met keeps what was read and is reported as `HATCH_STRUCTURE`. Its elevation (the z of its elevation point, group 30) and extrusion (210) are read from the header, and its fill style (75 -- normal, outermost only, or islands ignored) after the paths; its seed points and boundary source objects are not carried: the model has no place for them.
...
## A value's storage is undone; a wipeout's polygon is taken out of pixels

Every string value -- text and names alike -- is carried with the file's storage of it undone, as the model's principles (§6.1) ask of a reader. A DXF line cannot hold a control character, so the format writes one in caret notation (a line break in a frame's text is `^J`, and `^ ` is the caret); a character outside the file's code page is written as a `\U+XXXX` or `\M+nXXXX` escape. The reader turns each back into the character, so the same drawing read from a DXF and from a DWG carries the same strings. An escape naming an ASCII character is left as written (no writer needs one, and undoing it could turn text into an MTEXT code), as is a `\M+` escape whose code page this crate cannot decode (1361, Johab). The text's own control codes -- `%%d`, MTEXT's `\P` -- stay as written: they are text, not storage. A frame written without group 11 is not turned from the x axis: its direction is (1, 0, 0).
